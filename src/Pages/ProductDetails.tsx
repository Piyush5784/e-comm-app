...
import { itemsProp } from "./Store";

const ProductDetails = () => {
  const { id } = useParams();
  const item = useRecoilValueLoadable<itemsProp>(allItems);
  ...
  if (item.state == "loading") {
    ...
    const product = item.contents.find((item: itemsProp) => item.id == id);
            ...
            <div className="d-grid gap-2 d-md-flex justify-content-md-start mt-3">
              {quantity === 0 ? (
                <button
                  ...
                  onClick={() => increaseCartQuantity(parseInt(product.id))}
                >
                  Add to cart
                </button>
              ) : (
