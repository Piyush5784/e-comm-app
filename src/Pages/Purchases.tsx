...
const Purchases = () => {
  ...
  const [isLoading, setIsLoading] = useRecoilState<boolean>(isLoad);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const purchasesData: CartItem[] = await firebase.getPurchases();
        ...
        {data.map((item: DataItem, index: number) => (
          <>
            ...
            >
              ...
              <ul className="d-flex">
                {item.CartItems.map((cartItem: CartItem, cartIndex: number) => (
                  <>
