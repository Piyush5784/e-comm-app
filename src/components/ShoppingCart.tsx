...
const ShoppingCart = ({ isOpen }: ShoppingCartProp) => {
  ...
  const networkConnectionCount = useRecoilValueLoadable<itemsProp>(ItemsAtom);
  ...
  let total: number = Math.floor(getTotal());
  ...
  function getTotal() {
    if (networkConnectionCount.state == "hasValue") {
      return cartItems.reduce((total, cartItem) => {
        const item = networkConnectionCount.contents.find(
          (item: contentsProp) => item.id === cartItem.id
        );
